...
class ListsSuite extends FunSuite {
  ...
  val negs  : List[Int] = List(-3, -2,-1,0,1,2)
  val zeros : List[Int] = List(-2,-1,0)
  val empty : List[Int] = List()
  val maxer : List[Int] = List(3,3,3)
  val rando : List[Int] = List(3, 7, 2)
  ...
  test("sum with zero") {
    ...
    test("sum of empty set") {
        assert(sum(empty) === 0)
    }
     test("max of empty set") {
       intercept[NoSuchElementException] {
         assert(max(empty) === Nil)
       }
    }

    test("repeated max numbers") {
      assert(sum(maxer) === 9)
    }
